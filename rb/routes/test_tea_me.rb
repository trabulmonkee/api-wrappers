...
class TS_BAU_TEA_ME < ParallelTestCases
    def setup
        @URL = "#{ENV['BAU_BASE_URL']}/tea-me"
        @DEFAULT_ERR_BODY = JSON.parse(ENV['DEFAULT_ERR_BODY']).to_json
        @EXPECTED_BODY = '{"message":"delightful"}'
        @EXPECTED_ALLOW_VALUES = ['HEAD', 'GET', 'OPTIONS']
    end
    ...
    def test_tea_me_get
        ...
        bool, msg = check_response_status_code(rsp=rsp, expected_status_code=418)
        assert bool, msg
