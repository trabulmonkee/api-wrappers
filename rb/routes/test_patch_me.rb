...
class TS_BAU_PATCH_ME < ParallelTestCases
    def setup
        @URL = "#{ENV['BAU_BASE_URL']}/patch-me"
        @DEFAULT_ERR_BODY = JSON.parse(ENV['DEFAULT_ERR_BODY']).to_json
        @EXPECTED_BODY = '{"message":"patched"}'
        @EXPECTED_ALLOW_VALUES = ['PATCH', 'OPTIONS']
    end
