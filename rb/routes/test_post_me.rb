...
class TS_BAU_POST_ME < ParallelTestCases
    def setup
        @URL = "#{ENV['BAU_BASE_URL']}/post-me"
        @DEFAULT_ERR_BODY = JSON.parse(ENV['DEFAULT_ERR_BODY']).to_json
        @EXPECTED_BODY = '{"message":"posted"}'
        @EXPECTED_ALLOW_VALUES = ['POST', 'OPTIONS']
    end
