...
class TS_BAU_DELETE_ME < ParallelTestCases
    def setup
        @URL = "#{ENV['BAU_BASE_URL']}/delete-me"
        @DEFAULT_ERR_BODY = JSON.parse(ENV['DEFAULT_ERR_BODY']).to_json
        @EXPECTED_BODY = '{"message":"deleted"}'
        @EXPECTED_ALLOW_VALUES = ['DELETE', 'OPTIONS']
    end
    ...
    def test_delete_me_delete
        rsp = BaseApiUtils.delete_it(@URL)  
        bool, msg = check_response_status_code(rsp=rsp, expected_status_code=202)
        assert bool, msg
